...
elsif ($model eq 'log_reg') {
        $mode = 'classification';
}
else {
        say "Unknown model type '$model'" ;
        usage(-1);
}
...
my $params;
if ($model eq 'lin_reg') {
        $params = train_ridge_regression($instances, $targets);
}
elsif ($model eq 'log_reg') {
        $params = irls($instances, $targets);
}
...
usage: $PROGRAM_NAME [OPTIONS] [INPUT]
    ...
    --model=MODEL           MODEL can be lin_reg or log_reg
